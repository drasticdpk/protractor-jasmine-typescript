...
export class commonPageHelper {
    ...
    static async verifyTextInH1(text: string) {
        const pageText = await commonPageObjects.getTextFromH1(text).getText();
        await console.log('Got text: ' + pageText);
    ...
    static async verifyTextInTable(text: string) {
        const pageText = await commonPageObjects.getTextFromTable(text).getText();
        await console.log('Got text: ' + pageText);
        expect(pageText).toEqual(text);
        await console.log('Verified text on page');
    }
    ...
    static async verifyUrlContain(currentUrl: string) {
        const getURL = await browser.getCurrentUrl();
        await console.log('Got page url: ' + getURL);
        expect(getURL).toContain(currentUrl);
        await console.log('Verified Current Url');
    }
